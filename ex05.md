...
2. \[API\] の画面に遷移するので API の一覧から **HandsonAPI** をクリックし、画面上部のメニューから \[**Settings**\] タブをクリックします

    <img src="images/apim_Change_endpoint.png" width="500px">

3. API の設定画面が表示されるので \[**Web service URL**\] の値を `movie-app-xyz` のエンドポイント `https://movie-app-xyz/api/ `に変更し、画面下部の \[**Save**\] ボタンをクリックします
