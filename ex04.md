...
8. アプリケーションへのインターネットからのアクセスを遮断します
    ...
    <img src="images/AppService_puplicDeny.png" width="700px">
...
6. \[**ネットワーク**\] の画面に遷移するので、\[**送信トラフィック**] ボックス中の \[**VNET 統合**\] をクリックします
...
3. \[**仮想ネットワークの作成**\] 画面の \[**基本**\] タブに遷移するので、各項目を以下のように設定します
    ...
    デプロイが完了すると \[**リソースに異動**\] ボタンが表示されるので、クリックして作成した仮想ネットワークのリソース画面に遷移します。
...
Application Gateway のリソースを新規作成し、作成した仮想ネットワークとサブネットを関連付け、アプリケーション `MoveApp-UI-XYZ` をバックエンドプールに追加します。
...
14. \[**ネットワーク**\] の画面に遷移するので、\[**受信トラフィック**] ボックス中の \[**アクセス制限**\] をクリックします

    <img src="images/AppService_Network_accessSetting.png" width="700px">

15. \[**アクセス制限**\] の画面に遷移するので、\[**サイトのアクセスとルール**\] の \[**メインサイト**\] タブの \[**+ 追加**\] ボタンをクリックします
...
この手順では作業を簡単にするために既定のドメインに App Service が提供する ".azurewebsites.net" を使用しましたが実際のシナリオでは Application Gateway とバックエンドの App Service の両方でカスタム ドメインを使用することが推奨されています。
...
### その他 :  サービス エンドポイントとプライベート エンドポイントについて

Microsoft Azureのサービスエンドポイントとプライベートエンドポイントは、仮想ネットワーク内でのアクセスを制限する機能ですが、それぞれ異なる特性と利用シーンがあります。

**サービスエンドポイント**は、Azure Storage や Azure SQL Database など、仮想ネットワークに直接デプロイできないサービスを仮想ネットワーク内の特定のサブネットからのみにアクセスを制限できる機能です。これにより、Azure バックボーンの最適化されたネットワークを介して直接安全にアクセスでき、Azure サービス リソースへのアクセスを仮想ネットワークのみに絞ることができます²。

一方、**プライベートエンドポイント**は、Private Link　サービスを利用する際に作成されるエンドポイントで、Azure サービスだけでなく独自サービスでも利用できます。プライベート　エンドポイントを使用することで、Azure サービスをインターネットから隔離し、データの漏洩リスクを大幅に削減することができます。また、特定のリソースに対してのみ接続を許可し、その他のリソースへの接続をブロックすることができます。

したがって、サービス エンドポイントとプライベート エンドポイントの選択は、特定の要件やセキュリティ要件に基づいて行われます。


## まとめ

この演習では、App Service 上のアプリケーションへのアクセスを仮想ネットワーク内からのみに制限する方法と、App Service 上のアプリケーションから仮想ネットワーク内のリソースに接続する方法を学習しました。
